<template>
  <section class="cta" aria-labelledby="cta-title">
    ...
    <!-- cards -->
    <div class="cards">
      <article v-for="(c, i) in cards" :key="i" class="card" :style="cardDelayStyle(i)">
...
// small stagger animation per card
...
.cta{
  ...
  box-shadow: 0 14px 36px rgba(6,24,44,.10);
}

/* decorative background */
.bg-decor{ position:absolute; inset:-6% -6% 0 -6%; z-index:0; pointer-events:none; }
.bg-decor .grid{
...
.bg-decor .blob{
  position:absolute; filter: blur(24px); opacity:.65; border-radius: 999px;
  animation: float 16s ease-in-out infinite;
}
.b1{ width:460px;height:460px;left:6%;top:-8%; background:#8ed8ff; }
.b2{ width:340px;height:340px;right:8%;top:-6%; background:#bfe9ff; animation-delay:1.2s; }

@keyframes float{ 0%,100%{transform:translateY(0)} 50%{transform:translateY(14px)} }
@media (prefers-reduced-motion: reduce){ .blob{ animation:none } }

/* ---------- header ---------- */
.cta-head{ position: relative; z-index: 1; text-align: center; max-width: 980px; margin-inline:auto; }
.eyebrow{
  display:inline-block; font-size: 12px; letter-spacing:.26em; text-transform: uppercase;
  color:#1a628e; background: rgba(255,255,255,.6);
  border:1px solid rgba(12,74,110,.12);
  border-radius: 999px; padding: 6px 10px; margin-bottom: 10px;
}
.cta h2{
  font-weight: 900; margin: 4px 0 6px;
  font-size: clamp(22px, 2.4vw, 28px);
}
.sub{
  color:#3e6b86; margin: 0 auto 16px; max-width: 70ch;
}

/* button */
.btn{
  display:inline-flex; align-items:center; gap:10px;
  padding: 10px 22px; border:1px solid #d7e6f3; border-radius: 12px;
  font-weight: 800; cursor:pointer;
  background:linear-gradient(180deg,#ffffff,#f6fbff);
  color:#0b4871;
  box-shadow: 0 6px 16px rgba(17,89,133,.16);
  transition: transform .18s ease, box-shadow .18s ease, background .18s ease, border-color .18s ease;
}
.btn:hover, .btn:focus-visible{
  transform: translateY(-2px);
  background:#ffffff; border-color:#c8dcee; outline:none;
}
.btn-icon{ display:inline-grid; place-items:center; width:18px; }
...
.card{
  ...
  border: 1px solid rgba(12,74,110,.12);
  box-shadow: 0 10px 24px rgba(6,24,44,.10);
...
.icon-badge{
  ...
  box-shadow: inset 0 1px 0 rgba(255,255,255,.7), 0 10px 18px rgba(6,24,44,.08);
}
.icon-img{
...
/* text */
.card-title{
  font-size: 18px; font-weight: 800; color:#0a4b6e; margin: 4px 0 2px;
}
.card-text{
  color:#46697f; line-height:1.7; margin: 0;
}

/* foot */
.card-foot{
  display:flex; align-items:center; gap:10px; margin-top: 8px; color:#2b6286;
}
...
.card:hover .chev{ transform: translateX(2px); opacity:.9 }
</style>
